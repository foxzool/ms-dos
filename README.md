...
| **实时卫星** | **真实卫星轨道渲染**：CelesTrak TLE（stations + visual 共 ~180 颗）+ SGP4 轨道传播，TEME→GMST→地固坐标转换；随场景时钟运转（暂停即冻结、倍速快进可见星座运转）；ISS 金色高亮 + 标签 + 未来一圈点状轨道线；`S` 键开关 |
...
| **时间轴** | **Cesium 风格底部时间轴**：自适应刻度（1 分钟 ~ 30 天跨度）+ 时刻/日期显示；拖动把手或点击跳转场景时间（拖动时自动暂停）；滚轮缩放时间跨度（2 分钟 ~ 14 天）；播放中窗口自动前滚，可回看任意时刻 |
...
- **Web 端（WebGL2）HUD 渲染限制（已修复主要问题）**：相机级 MSAA 会破坏 UI pass、UI 挂在失活相机上会整体消失——两处均已通过相机级 `Msaa::Off` 与 UI 相机标记随状态迁移修复（2026-09）；时间轴把手颜色在 WebGL2 下偏暗（对比度仍清晰）。瓦片磁盘缓存可显著减少重复请求，也可手动预置缓存文件（`~/.cache/ms-dos/tiles/tile_{gx}_{gy}.xml`，0.1° 网格键）；单位在世界坐标下的 f32 表征在约 ±1.5m 精度，极高倍放大下可能有轻微量化（战术缩放级别无感）。
